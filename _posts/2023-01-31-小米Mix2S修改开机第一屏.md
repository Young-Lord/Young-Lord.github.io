...
下载[这个文件](https://young-lord.github.io/files/mix2s_logo_modify.py)，在与此文件相同目录放置以下文件（不放置则表示不替换此项）。**注意，此处的bmp文件尺寸必须与原文件尺寸一致，色深必须为24bit！**
...
bmp 文件是不压缩的，因此根据尺寸、色深（此处是24bit）就可以确认所需存储空间。
