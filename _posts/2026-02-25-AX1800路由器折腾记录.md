...
值得一提的是，我位于“静态 IP 上网楼栋”，不同楼栋间可能存在区别。比如，部分楼栋分配到的 IPv4 地址可能改变，部分楼栋可能没有 IPv6 支持。
...
目标：通过内网设备分配到的`2403:d400::/32`下的 IPv6 地址，从教育网内外访问路由器下设备。支持更改转发前后的端口号。

```console
$ vi /etc/firewall.user

## 只允许特定 IP / 协议 / 端口 的入站连接
# ip6tables -D forwarding_rule -d 2403:d400:1400:1234::123 -p tcp --dport 22 -j ACCEPT
# ip6tables -I forwarding_rule -d 2403:d400:1400:1234::123 -p tcp --dport 22 -j ACCEPT

## 允许所有 IPv6 的入站连接
# ip6tables -D forwarding_rule -j ACCEPT 2>/dev/null
# ip6tables -I forwarding_rule -j ACCEPT 2>/dev/null

## 更改转发前后的端口号
# ip6tables -t nat -D PREROUTING -d 2403:d400:1400:1234::123 -p tcp --dport 33322 -j DNAT --to-destination [2403:d400:1400:1234::123]:22 2>/dev/null
# ip6tables -t nat -I PREROUTING -d 2403:d400:1400:1234::123 -p tcp --dport 33322 -j DNAT --to-destination [2403:d400:1400:1234::123]:22
## 放行流量。由于 NAT 转换发生在路由之前，所以这里要放行的是“转换后”的真实内部端口（22）
# ip6tables -D forwarding_rule -d 2403:d400:1400:1234::123 -p tcp --dport 22 -j ACCEPT 2>/dev/null
# ip6tables -I forwarding_rule -d 2403:d400:1400:1234::123 -p tcp --dport 22 -j ACCEPT
```

随后`/etc/init.d/firewall restart`。

## DDNS

也就是用 Cloudflare API 定期更新 DNS 记录。由于当前网络环境下 IP 地址基本固定，暂时不设置。
