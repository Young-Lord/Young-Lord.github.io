...
# for Arch Linux ARM
# pacman -S --needed autoconf make git automake pkg-config libtool gcc wget
...
收集足够nounces（即停止显示`Collected xxx nounces...`，进入到`Cracking`，或提示`press enter to start brute-force phase`）后，你就可以直接停止破解，在目录下找到`0x1111aaaa_003A.txt`的文件（`1111aaaa`为卡片UID，`3`为待破解block（注意不是sector！比如sector 11包含44-47这4个block），`A`为待破解扇区密钥类型）。
